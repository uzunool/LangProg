...
class Overl {
	...
	Overl(int i) {
		System.out.println("Внутри конструктора с одним целым парметром");
		s = i;
	}
	Overl(double i) {
		System.out.println("Внутри конструктора с одним вещественным парметром");
                s = (int) i; 
	}
	Overl(int i, int j) {
		System.out.println("Внутри конструктора с двумя целым парметрами");
                s = i + j;
	}
	void ovlDemo() {
